...
const FooterBar = () => {
  ...
  const normalizeFooterHtml = (value) =>
    sanitizeHtmlContent(value || '').replace(/\bnew[\s_-]*api\b/gi, 'icucode');
  const [footer, setFooter] = useState(() =>
    normalizeFooterHtml(getFooterHTML()),
  );
  const systemName = getSystemName();
  const displaySystemName = /\bnew[\s_-]*api\b/i.test(systemName)
    ? 'icucode'
    : systemName;
  const logo = getLogo();
  ...
  const loadFooter = () => {
    ...
    if (footer_html) {
      setFooter(normalizeFooterHtml(footer_html));
    }
  };
  ...
  const customFooter = useMemo(
    () => (
      <footer className='relative h-auto py-16 px-6 md:px-24 w-full flex flex-col items-center justify-between overflow-hidden'>
        ...
        <div className='flex flex-col md:flex-row items-center justify-center w-full max-w-[1110px] gap-6 text-center'>
